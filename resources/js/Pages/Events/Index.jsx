...
export default function Index({ events, registeredEventIds = [] }) {
  ...
  // Form state for Admin On-the-Spot pendaftaran peserta hari-H
  ...
  // On-the-Spot: buka modal pendaftaran + presensi peserta walk-in hari-H
  const openOnSpotModal = (eventId) => {
    setSelectedEventId(eventId);
    onSpotForm.setData({
      name: '',
      email: '',
  ...
  return (
    <AppLayout title="Katalog BIMTEK">
      <div className="space-y-6 max-w-7xl mx-auto font-sans">
              ...
              <div className="pt-3 border-t border-slate-100">
                {isAdmin ? (
                  <div className="space-y-2">
                    ...
                    {/* PESERTA ON-THE-SPOT HARI-H */}
                    <button
                      type="button"
                      onClick={() => openOnSpotModal(item.id)}
                      className="w-full py-2 px-3 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-extrabold flex items-center justify-center gap-1.5 transition-colors"
                    >
                      <UserPlus className="w-4 h-4" />
                      <span>Peserta On-the-Spot (Hari-H)</span>
                    </button>
        ...
        {/* MODAL PESERTA ON-THE-SPOT HARI-H */}
        {showOnSpotModal && (
              ...
              <form onSubmit={handleOnSpotSubmit} className="space-y-3.5 text-xs">
                ...
                <div className="flex items-center justify-end gap-2 pt-3 border-t border-slate-200">
                  ...
                  <button
                    ...
                    className="px-5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-lg flex items-center gap-1.5 disabled:opacity-50"
                  >
                    <UserPlus className="w-4 h-4" />
